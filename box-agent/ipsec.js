...
function _reloadStrongswan() {
  // Try the modern unit name first, then the legacy one
  ...
  } else if (sh('systemctl is-active strongswan 2>/dev/null') === 'active') {
    sh('systemctl reload strongswan || systemctl restart strongswan');
  } else {
    // Start either, prefer -starter for the ipsec.conf style we're using
    sh('systemctl enable --now strongswan-starter 2>/dev/null || systemctl enable --now strongswan 2>/dev/null');
  }
  // Also try ipsec rereadall / update for live reload of secrets
